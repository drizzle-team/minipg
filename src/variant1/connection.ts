...
import { W, Parser, parseRowDescription, parseDataRow } from './protocol.ts'
...
function makeRow(cells: (Buffer | null)[], body: Buffer, mode: ResultMode, fields: Field[], colDecoders: Decoder[]): unknown {
  switch (mode) {
    ...
    case 'object': {
      ...
      for (let i = 0; i < fields.length; i++) { const f = fields[i]!; const c = cells[i]; o[f.name] = c == null ? null : colDecoders[i]!(c) }
      return o
    }
    default: { // 'array'
      const r = new Array(fields.length)
      for (let i = 0; i < fields.length; i++) { const c = cells[i]; r[i] = c == null ? null : colDecoders[i]!(c) }
      return r
    }
  }
}
...
export class Connection {
  ...
  private prepared = new Map<string, { sql: string; fields: Field[]; colDecoders: Decoder[] }>()
  ...
  private handle(type: string, body: Buffer): void {
    switch (type) {
      ...
      case 'T': if (this.current) { this.current.fields = parseRowDescription(body); this.current.colDecoders = this.current.fields.map((f) => decoderFor(f.dataTypeOid, this.cfg.decoders)); if (this.current._cacheName) this.prepared.set(this.current._cacheName, { sql: this.current.sql, fields: this.current.fields, colDecoders: this.current.colDecoders }) } return
      case 'n': if (this.current) { this.current.fields = []; this.current.colDecoders = []; if (this.current._cacheName) this.prepared.set(this.current._cacheName, { sql: this.current.sql, fields: [], colDecoders: [] }) } return
  ...
  private dataRow(body: Buffer): void {
    ...
    try {
      ...
      row = makeRow(cells, body, t.mode, t.fields ?? [], t.colDecoders ?? [])
  ...
  private startTask(t: Task): void {
    ...
    try {
      ...
      if (t.name) {
        ...
        if (cached && cached.sql === t.sql) { reuse = true; t.fields = cached.fields; t.colDecoders = cached.colDecoders }
