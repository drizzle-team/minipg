...
export class Connection {
  ...
  private dataRow(body: Buffer): void {
    ...
    // Fused single pass: read the DataRow body directly and build the row without an
    // intermediate cells[] array. Behavior is identical to parseDataRow + makeRow.
    try {
      const mode = t.mode
      if (mode === 'raw') { row = Buffer.from(body) }
      else {
        const decoders = this.cfg.decoders
        ...
        if (mode === 'buffer') {
          ...
          for (let i = 0; i < n; i++) {
            const len = body.readInt32BE(off); off += 4
            const f = fields[i]!
            if (len === -1) o[f.name] = null
            else { o[f.name] = decoderFor(f.dataTypeOid, decoders)(body.subarray(off, off + len)); off += len }
          }
          row = o
        } else { // 'array'
          const r = new Array(n)
          for (let i = 0; i < n; i++) {
            ...
            const f = fields[i]!
            if (len === -1) r[i] = null
            else { r[i] = decoderFor(f.dataTypeOid, decoders)(body.subarray(off, off + len)); off += len }
          }
          row = r
        }
      }
