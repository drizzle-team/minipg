...
export class Connection {
  ...
  private dataRow(body: Buffer): void {
    ...
    if (!t || t.cancelled || t.error) return
    let row: unknown
    ...
    try {
      ...
      row = makeRow(cells, body, t.mode, t.fields ?? [], this.cfg.decoders)
  ...
  query(sql: string, params: unknown[] = [], opts: QueryOptions = {}): Promise<QueryResult<never>> {
    return new Promise<QueryResult<never>>((resolve, reject) => {
      ...
      const task: Task = { sql, params, name: opts.name, mode: opts.mode ?? 'array', rows: [], resolve, reject }
  ...
  stream<Row = unknown[]>(sql: string, params: unknown[] = [], opts: StreamOptions = {}): AsyncIterableIterator<Row> {
    ...
    const t: Task = {
      sql, params, name: opts.name, mode: opts.mode ?? 'array', stream: true, rows: [],
      onRow(row) {
        buf.push(row as Row)
    ...
    else { this.queue.push(t); this.processQueue() }
    return {
