...
import { W, Writer, Parser, parseRowDescription } from './protocol.ts'
...
interface Task {
  ...
  rows: unknown[]
  fields?: Field[]
  colDecoders?: Decoder[] // per-column decoder resolved once from fields (parsed modes only)
  command?: string | null
...
// Fused parse + decode: one pass over the DataRow body, no intermediate cells array.
// 'raw' returns the whole body copy; 'buffer' copies each cell; the parsed modes
// ('object'/'array') decode inline using the per-column decoders resolved once per task.
// Byte-for-byte equivalent to baseline parseDataRow()+makeRow().
function decodeRow(body: Buffer, mode: ResultMode, fields: Field[], colDecoders: Decoder[]): unknown {
  ...
  if (mode === 'buffer') {
    ...
    for (let i = 0; i < n; i++) {
      ...
      if (len === -1) r[i] = null
      else { r[i] = Buffer.from(body.subarray(off, off + len)); off += len }
    }
    return r
  }
  // object/array iterate fields.length (matching baseline makeRow): a missing cell
  // (i >= n) decodes to null, mirroring cells[i] === undefined in the baseline.
  if (mode === 'object') {
    const o: Record<string, unknown> = Object.create(null) // null-proto: a column named __proto__ can't pollute
    for (let i = 0; i < fields.length; i++) {
      const name = fields[i]!.name
      if (i >= n) { o[name] = null; continue }
      const len = body.readInt32BE(off); off += 4
      if (len === -1) o[name] = null
      else { o[name] = colDecoders[i]!(body.subarray(off, off + len)); off += len }
    }
    return o
  }
  // 'array'
  ...
  for (let i = 0; i < fields.length; i++) {
    if (i >= n) { r[i] = null; continue }
    const len = body.readInt32BE(off); off += 4
    if (len === -1) r[i] = null
    else { r[i] = colDecoders[i]!(body.subarray(off, off + len)); off += len }
  }
  return r
}
...
export class Connection {
  ...
  private writer = new Writer() // reusable serialization buffer for the per-query message burst
  private queue: Task[] = []
  ...
  private handle(type: string, body: Buffer): void {
    switch (type) {
      ...
      case 'T': if (this.current) { this.current.fields = parseRowDescription(body); if (this.current._cacheName) this.prepared.set(this.current._cacheName, { sql: this.current.sql, fields: this.current.fields }) } return
  ...
  private dataRow(body: Buffer): void {
    ...
    try {
      ...
      // Resolve per-column decoders once per task (only for the parsed modes that use them).
      let colDecoders = t.colDecoders
      if (!colDecoders && (t.mode === 'array' || t.mode === 'object')) {
        colDecoders = t.colDecoders = fields.map((f) => decoderFor(f.dataTypeOid, this.cfg.decoders))
      }
      row = decodeRow(body, t.mode, fields, colDecoders ?? [])
  ...
  private startTask(t: Task): void {
    ...
    try {
      ...
      const name = t.name ?? ''
      let reuse = false
      const w = this.writer.reset() // safe to reuse: one query in flight; prior write has flushed
      if (t.name) {
        const cached = this.prepared.get(t.name)
        if (cached && cached.sql === t.sql) { reuse = true; t.fields = cached.fields }
        else if (cached) { w.close('S', t.name); this.prepared.delete(t.name) }
      }
      if (!reuse) { w.parse(name, t.sql).describe('S', name); if (t.name) t._cacheName = t.name }
      w.bind('', name, enc, 0).execute('', 0).sync()
      payload = w.take()
    } catch (e) {
