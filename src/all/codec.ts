...
// int2/int4/oid are always small enough to fit a JS number exactly; parse the ASCII
// digits straight from the bytes to avoid the intermediate string + Number() call.
// Behaviour matches Number(b.toString('utf8')) for valid integer text.
const asInt: Decoder = (b) => {
  let i = 0
  let neg = false
  if (b[0] === 0x2d) { neg = true; i = 1 } // '-'
  else if (b[0] === 0x2b) { i = 1 } // '+'
  let n = 0
  for (; i < b.length; i++) n = n * 10 + (b[i]! - 48)
  return neg ? -n : n
}
